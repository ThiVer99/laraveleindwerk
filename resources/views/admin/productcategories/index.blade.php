...
@section('content')
    <div class="d-flex justify-content-between shadow-lg p-3 mb-5 bg-body-tertiary rounded">
        <div class="d-flex">
            <p class="rounded bg-danger m-0 d-flex align-self-center p-2 text-white">{{$productcategories->total()}}</p>
        ...
        <a href="{{route('productcategories.create')}}" class="btn btn-primary m-2 rounded-pill">Add Product category</a>
    </div>
    @if (session('alert'))
    ...
    <table class="table table-striped shadow-lg p-3 mb-5 bg-body-tertiary rounded">
        ...
        @foreach($productcategories as $productcategory)
            <tr @if($productcategory->deleted_at != null) class="bg-danger text-white" @endif>
                ...
                <td>
                    <div class="nav-item dropdown no-arrow">
                        <a class="nav-link dropdown-toggle" href="#" id="userDropdown{{ $productcategory->id }}" role="button"
                        ...
                        <div class="dropdown-menu  shadow "
                             aria-labelledby="userDropdown{{ $productcategory->id }}">
                            <a class="dropdown-item" href="{{route('productcategories.edit', $productcategory->id)}}">
                            ...
                            @if($productcategory->deleted_at != null)
                                <form action="{{ route('productcategories.restore', $productcategory->id) }}" method="POST">
                                    @csrf
                                    @method('POST')
